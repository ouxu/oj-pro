...
export default class Ranklist extends React.PureComponent {
  ...
  componentWillReceiveProps (nextProps) {
    if (nextProps.location.query.scope !== this.props.location.query.scope) {
      this.setState({data: []})
      this.fetchData(1, nextProps.location.query.scope)
    }
  }
      ...
      <div className='ranklist'>
        <div className='page-header'>
          ...
          <div className='page-header-description' >
            没啥用的排行榜，AC：正确数，ANS：提交数，仅显示前200
          </div>

        </div>
        <div className='page-header-extra'>
          <Tabs
            activeKey={this.props.location.query.scope || 'total'}
            className='tabs'
        ...
        <div className='m-16 px-16 bg-white ranklist-content'>
          <List
            ...
            renderItem={(item, index) => (
              <List.Item
                actions={[(<div>AC：{item.solved}</div>), (<div>ANS：{item.submit}</div>)]}
              >
